...
class RoboHandler:
  ...
  start=self.start_trans
        goals=goal_transforms
	while not self.is_at_goal_basesearch(start,goals,dist_thresh=0.2,theta_thresh=np.pi/12)
		new_trans=self.transition_config(start)
    return None
  ...
  def init_transition_transforms(self):
    ...
    temp_full_transforms = []
    for w1 in range(0.1,1,0.1):
	for w2 in range(0.1,1,0.1):
		for t in range(0.1,1,0.1):
			init_array=np.array([0,0,0])
			for i in range(0,t,TIMESTEP_AMOUNT):
				theta = init_array[2]
				temp = np.array([((w1/2)*WHEEL_RADIUS*cos(theta))+((w2/2)*WHEEL_RADIUS*cos(theta)), ((-w1/2)*WHEEL_RADIUS*sin(theta))+((-w2/2)*WHEEL_RADIUS*sin(theta)),((w1*WHEEL_RADIUS)/(2*ROBOT_LENGTH))-((w2*WHEEL_RADIUS)/(2*ROBOT_LENGTH))])
				init_array = np.add(init_array,temp)
                                temp_full_transforms.append(params_to_transform(init_array))
                        self.full_transforms.append(temp_full_transforms)
			self.transition_transforms.append(params_to_transform(init_array))	
  ...
  def transition_config(self, config):
	new_trans=[]
	for trans in transition_transforms
		new_trans.append(np.dot(config,trans))
    return new_trans
